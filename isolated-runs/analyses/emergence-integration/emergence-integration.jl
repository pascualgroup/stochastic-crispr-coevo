...
dbOutputPath = joinpath("emergence-integration_output.sqlite") # cluster
...
if isfile(dbOutputPath)
    error("emergence-integration.sqlite already exists; delete first")
end
##
...
function potentialStructure!(matchStructure::modHierarchy)
...
function findEscapeLineages!(matchStructure::modHierarchy,matchIDs::Dict{Vector{Int64},Int64})
    dbTempSim = matchStructure.dbSim
    dbTempMatch = matchStructure.dbMatch
    dbTempTri = matchStructure.dbTri
...
function findImmuneStrains!(matchStructure::modHierarchy)
    dbTempSim = matchStructure.dbSim
    dbTempTri = matchStructure.dbTri
    ...
    for matchID in [keys(matchStructure.matches)...]
        for pheno in matchStructure.matches[matchID]
            if matchID in keys(matchStructure.ibstrains)
                union!(matchStructure.ibstrains[matchID],
                            matchStructure.ispacers[pheno])
            else
                matchStructure.ibstrains[matchID] =
                    matchStructure.ispacers[pheno]
            end
        end
    end
...
function microbeBiomass!(matchStructure::modHierarchy,current::intState,pstate::laststate)
    time = current.time
    dbTempSim = matchStructure.dbSim
    dbTempTri = matchStructure.dbTri
    ...
    for matchID in [0,keys(matchStructure.matches)...]
        ...
        sStrains = setdiff(bstrains,immuneStrains)
        if length(sStrains) > 0
...
function emergence(matchStructure::modHierarchy,
                    current::intState,pstate::probstate,parameters::params,dt::Float64)
    extinction = computeActualRoot!(matchStructure,current,pstate,parameters)
...
function escape(pActualExpectation,pIntExpectation,matchStructure,matchIDs)
    ...
    for (t,) in execute(dbTempSim,"SELECT DISTINCT t FROM vabundance")
        ...
        microbeBiomass!(matchStructure,current,pstate)
