...
run_id = ARGS[1]
mutationProbs= map(x->parse(Float64,x),ARGS[2:end])

## Define Paths ##
SCRIPT_PATH = abspath(dirname(PROGRAM_FILE))
#
dbSimPath = joinpath(SCRIPT_PATH,"..","..","..","simulation","sweep_db_gathered.sqlite") # cluster
dbOutputPath = joinpath("experiment-lambert-root_output.sqlite") # cluster
...
function assemble(current::state, extinction::pcomponents,escapes::Dict{Int64,Vector{Int64}},parameters::params)
    time = current.time
    ...
    DataFrame(t=fill(Float64(time), length(current.matches)),
        viral_mutation_prob = parameters.viral_mutation_rate, 
...
function emergence!(matchStructure::hierarchy,
    ...
    assemble(current,extinction,matchStructure.escapes,parameters)
...
function escape()
    ...
    for (t,) in execute(dbTempSim,"SELECT DISTINCT t FROM vabundance")
        ...
        for mutProb in mutationProbs
            parameters.viral_mutation_rate = mutProb
