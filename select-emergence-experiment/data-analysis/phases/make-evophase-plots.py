...
#PLOT_PATH = os.path.join(SCRIPT_PATH,'..','gathered-analyses','phase','diagram') # cluster
...
#DBEXTINCT_PATH = os.path.join(SCRIPT_PATH,'..','gathered-analyses','extinctions','extinctions.sqlite') # cluster
#DBPEAKS_PATH = os.path.join('/Volumes','Yadgah','extinctions.sqlite') # local
#DBPEAKS_PATH = os.path.join('/Volumes','Yadgah','extinctions_output.sqlite') # local. run_id fixed; for testing
...
thresholds = curWalls.execute('SELECT upper_threshold,lower_threshold,hill2_threshold FROM threshold_values WHERE run_id = {}'.format(run_id)).fetchall()
...
combos = curSim.execute('SELECT DISTINCT combo_id FROM param_combos').fetchall()

for combo_id in combos
    curSim.execute('SELECT combo_id FROM param_combos').fetchall()
