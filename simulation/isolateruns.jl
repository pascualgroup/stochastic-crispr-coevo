...
dbSimPath = joinpath(SCRIPT_PATH,"sweep_db_gathered.sqlite") # cluster
...
function main()
    ...
    run_ids = [1]
    # execute(dbSim, "BEGIN TRANSACTION")
    # execute(dbSim, "CREATE INDEX summary_index ON summary (t,microbial_abundance,viral_abundance,run_id)")
    # execute(dbSim, "CREATE INDEX babund_index ON babundance (t,bstrain_id,abundance,run_id)")
    # execute(dbSim, "CREATE INDEX vabund_index ON vabundance (t,vstrain_id,abundance,run_id)")
    # execute(dbSim, "CREATE INDEX bspacers_index ON bspacers (bstrain_id,spacer_id,run_id)")
    # execute(dbSim, "CREATE INDEX vpspacers_index ON vpspacers (vstrain_id,spacer_id,run_id)")
    # execute(dbSim, "CREATE INDEX bstrain_index ON bstrains (t_creation,bstrain_id,parent_bstrain_id,infecting_vstrain_id,run_id)")
    # execute(dbSim, "CREATE INDEX vstrain_index ON vstrains (t_creation,vstrain_id,parent_vstrain_id,infected_bstrain_id,run_id)")
    # execute(dbSim, "COMMIT")

    for run in run_ids
        println("Isolating run $(run)...")
        (cr,) = execute(dbSim,"SELECT combo_id,replicate FROM runs WHERE run_id = $(run)")
        repDir = "run_id$(run)_combo$(cr[1])_replicate$(cr[2])"
        mkpath(joinpath(SCRIPT_PATH,"isolated-runs",repDir))
