...
execute(dbOutput, "CREATE TABLE matches_spacers (t REAL, time_specific_match_id INTEGER, spacer_id INTEGER)")
...
# @time begin
    ...
    function identifyMatches()
        for (time,) in execute(dbTemp, "SELECT DISTINCT t FROM babundance ORDER BY t")
            println("Searching matches at time $(time)")
            match_id = 1
            if time > 30.0
                return
            end
            ...
            for (bstrain_id,) in execute(dbTemp, "SELECT bstrain_id FROM babundance
                ...
                matchedSpacers, match_id, vstrainMatches =
                bstrainTOvstrain(time,bstrain_id,match_id,vstrainMatches)
    ...
    function bstrainTOvstrain(time,bstrain_id,match_id,vstrainMatches::Dict)
        ...
        for (vstrain_id,) in execute(dbTemp, "SELECT vstrain_id FROM vabundance
            ...
            for spacer_id in spacers
                execute(dbOutput, "INSERT INTO matches_spacers VALUES (?,?,?)",(time, match_id, spacer_id))
            end
        ...
        return matchedSpacers, match_id, vstrainMatches
    end
