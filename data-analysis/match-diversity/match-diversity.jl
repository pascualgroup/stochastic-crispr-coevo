...
dbOutputPath = joinpath("/Volumes/Yadgah/match_diversity_output.sqlite") # local
...
execute(dbTemp, "CREATE TABLE bspacers (t REAL, vstrain_id INTEGER, abundance INTEGER)")
...
execute(dbTemp, "CREATE INDEX vspacers_index ON vpspacers (vstrain_id,spacer_id)")
    #execute(dbTemp, "CREATE INDEX bstrains_index ON bspacers (t_creation,bstrain_id,parent_bstrain_id,infecting_vstrain_id)")
    #execute(dbTemp, "CREATE INDEX vstrains_index ON vpspacers (t_creation,vstrain_id,parent_vstrain_id,infected_bstrain_id)")
execute(dbTemp, "COMMIT")
