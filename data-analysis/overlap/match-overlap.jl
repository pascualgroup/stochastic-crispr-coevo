...
dbOutputPath = joinpath("/Volumes/Yadgah/overlap_output.sqlite") # local

if isfile(dbOutputPath)
    error("overlap_output.sqlite already exists; delete first")
end # cluster
##
...
execute(dbOutput, "CREATE TABLE bstrain_overlap (t REAL, bstrain_id1 INTEGER, bstrain_id2 INTEGER,
overlap_id)")

execute(dbOutput, "CREATE TABLE overlap_ids (t REAL, overlap_id INTEGER,
spacer_id INTEGER)")
