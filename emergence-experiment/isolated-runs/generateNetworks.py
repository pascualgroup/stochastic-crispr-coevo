...
def generate(run_id, combo_id, replicate,
    ...
    for time in times:
        ...
        vTotal = virus_total[virus_total.t ==
                             time]['Viral Abundance'].values[0]
        vAbunds = pd.read_sql_query("SELECT vstrain_id,abundance FROM vabundance \
        WHERE run_id = {0} AND t = {1}".format(run_id, time), conSim)
        bTotal = microbe_total[microbe_total.t ==
                               time]['Microbial Abundance'].values[0]
        ...
        bmatchcollapse['prob'] = bmatchcollapse['prob'] * \
            1/bTotal*np.array(bmatchcollapse['bmatchAbund'])
        bmatchcollapse = bmatchcollapse.sort_values(
            by=['prob'])  # bin values are sorted
        # This adds the bmatchID that was not originally in the infection network
        bstrainsOrdered = list(
            set(bstrainmatchIDs['bmatch_id']) - set(bmatchcollapse['bmatch_id']))
        ...
        vmatchAbund = vmatchAbund.sort_values(
            by=['EscEventProb'])  # bin values are sorted here
        ###
        ...
        tripartiteNetwork['spacer_id'] = [spacerDict[i]
                                          for i in tripartiteNetwork['spacer_id']]
        ...
        #
        tripartiteNetwork = tripartiteNetwork.pivot(
            index='vmatch_id', columns='bmatch_id', values='spacer_id')
        tripartiteNetwork = tripartiteNetwork.reindex(
            index=vmatchAbund['vmatch_id'])  # reordered here
        # reordered here
        tripartiteNetwork = tripartiteNetwork[bstrainsOrdered]
        microbeBounds = [0]
        mtickLocation = []
        ###
        ...
        for i in bstrainsOrdered:
            j = bmatchAbund[bmatchAbund.bmatch_id ==
                            i]['bmatchAbund'].values[0]
            microbeBounds.append(microbeBounds[-1]+np.log(j)+1)
            mtickLocation.append(
                microbeBounds[-2]+(microbeBounds[-1]-microbeBounds[-2])/2)
        ...
        for i in vmatchAbund['vmatchAbund']:
            ...
            vtickLocation.append(
                virusBounds[-2]+(virusBounds[-1]-virusBounds[-2])/2)
        ...
        heat = ax.pcolormesh(microbeBounds, virusBounds, tripartiteNetwork,
                             cmap=spacerCMAP, edgecolors='black', linewidth=0.01)
        ...
        ax.set_title('runID{0}-c{1}-r{2}\nTripartite Match Network at t = {3} (contact-ordered)'.format(
            run_id, combo_id, replicate, time), pad=20, fontsize=10)
        fig.tight_layout()
        for imgType in imgTypes:
            fig.savefig(os.path.join(PLOT_PATH, 'tripartite-match-network-contact-ordered-time{0}.{1}'.format(
                int(np.floor(time)), imgType)), dpi=resolve)
        plt.close(fig)
        ###
        # Infection Networks
        ...
        bmatchvmatch0 = bmatchvmatch0.pivot(
            index='vmatch_id', columns='bmatch_id', values='presence')
        ...
        microbeBounds = list(1/max(microbeBounds)*np.array(microbeBounds))
        fig, ax = plt.subplots(1)
        ax.pcolormesh(microbeBounds, virusBounds, bmatchvmatch0,
        ...
        ax.set_xlabel(xlabel='Microbe Match ID', labelpad=10)
        ax.set_ylabel(ylabel='Virus Match ID', labelpad=10)
        ax.set_title('runID{0}-c{1}-r{2}\nInfection Network of Match Types at t = {3} (contact-ordered)'.format(
            run_id, combo_id, replicate, time), pad=20, fontsize=10)
        fig.tight_layout()
        for imgType in imgTypes:
            fig.savefig(os.path.join(PLOT_PATH, 'infection-network-match-types-contact-ordered-time{0}.{1}'.format(
                int(np.floor(time)), imgType)), dpi=resolve)
        plt.close(fig)
        ##
        ...
        bstrainvstrain0 = bstrainvstrain0.pivot(
            index='vstrain_id', columns='bstrain_id', values='presence')
        bstrainvstrain0 = bstrainvstrain0.reindex(
            index=vstraincollapse['vstrain_id'])
        ...
        virusBounds = list(1/max(virusBounds)*np.array(virusBounds))
        fig, ax = plt.subplots(1)
        ax.pcolormesh(microbeBounds, virusBounds, bstrainvstrain0,
                      edgecolors='black', linewidth=0.01)
        ax.set_xticks(mtickLocation)
        ax.set_xticklabels(
            bstraincollapse['bstrain_id'], rotation=0, fontsize=4)
        ax.set_yticks(vtickLocation)
        ax.set_yticklabels(
            vstraincollapse['vstrain_id'], rotation=0, fontsize=3.5)
        ax.set_xlabel(xlabel='Microbe Strain ID', labelpad=10)
        ax.set_ylabel(ylabel='Virus Strain ID', labelpad=10)
        ax.set_title('runID{0}-c{1}-r{2}\nInfection Network of Strains at t = {3} (contact-ordered)'.format(
            run_id, combo_id, replicate, time), pad=20, fontsize=10)
        fig.tight_layout()
        for imgType in imgTypes:
            fig.savefig(os.path.join(PLOT_PATH, 'infection-network-strains-contact-ordered-time{0}.{1}'.format(
                int(np.floor(time)), imgType)), dpi=resolve)
        plt.close(fig)
