    ...
    for i in 1:length(coarseSeries)
        ...
        if coarseSeries[i] == 0 && coarseSeries[i+1] == 1
            if check == 3
                ...
                # println("timeStmt is $(timeStmt)")
                bshannon = [shannon.bshannon for shannon in execute(dbTempShan, "SELECT bshannon FROM shannon_diversity 
                                                                WHERE run_id = $(run_id) AND t in ($(timeStmt...)) ORDER BY t")]
                vshannon = [shannon.vshannon for shannon in execute(dbTempShan, "SELECT vshannon FROM shannon_diversity 
                                                                WHERE run_id = $(run_id) AND t in ($(timeStmt...)) ORDER BY t")]
                spacerDiv = [div for (div,) in execute(
                    dbTempTri,
                    "SELECT shannon_diversity FROM single_spacer_match_diversity 
                    WHERE run_id = $(run_id) AND t > $(peakSeries.t[j1]) AND t < $(peakSeries.t[j4])")]
                if maximum(bshannon) >= shannonThreshold
                    walls = walls + 1
                    wallSeries.wall_presence[j1:j4] = Array{Int64,1}(ones(length(j1:j4)))
                    # println("bshannon is $(bshannon[:, :])")
                    # println("bshannon length is $(length(bshannon[:, :]))")
                    # println("wallSeries.bshannon[j1:j4] is $(wallSeries.bshannon[j1:j4])")
                    # println("wallSeries.bshannon[j1:j4] length is $(length(wallSeries.bshannon[j1:j4]))")
