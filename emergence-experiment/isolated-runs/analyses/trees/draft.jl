...
# dbOutputPath = joinpath("/Users/armun/Dropbox/Current/Projects/microbe-virus-crispr/stochastic-crispr/
#                                 trees_output.sqlite") # local
...
function MRCAtimeSeries(daughter_id::Int64,
                        lineages::Dict{Int64,Array{Int64,1}},strainType::String)
    if strainType == "virus"
        type = "v"
    end
    if strainType == "microbe"
    ...
    execute(dbOutput, "CREATE TABLE MRCA_$(s)strains (t REAL, lineage INTEGER,
                            MRCA_$(s)strain_id INTEGER, t_creation REAL)")
    ...
    for (time,) in execute(dbTemp,"SELECT DISTINCT t FROM $(s)abundance ORDER BY t")
        ...
        strains = strains[strain .!= initialStrains]
        if length(strains) == 0
            continue
        end
