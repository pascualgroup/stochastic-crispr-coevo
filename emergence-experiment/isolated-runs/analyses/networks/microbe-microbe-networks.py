...
def microbe2microbe(times):
    for time in times:
print('Compiling tripartite (microbe-microbe) networks at time {0}'.format(time))
...
bstrains=[bstrain_id[0] for bstrain_id in
    ...
    WHERE t = {0} \
    AND vstrain_id in ({1})"
    .format(time,', '.join(map(str,vstrains0))))]
...
nonMatchDF = nonMatchDF.rename(columns={"bstrain_id": "bstrain_id_0"})
singleMatchDF = pd.read_sql_query(
    "SELECT vstrain_id, bstrain_id \
    FROM bstrain_to_vstrain_matches WHERE t = {0} AND match_length = 1 \
    AND vstrain_id in ({1}) ORDER BY time_specific_match_id".format(time,
    ', '.join(map(str,vstrains0))), conMatch)
singleMatchDF = singleMatchDF.rename(columns={"bstrain_id": "bstrain_id_1"})
nonMatchDF = nonMatchDF.astype(int)
singleMatchDF = singleMatchDF.astype(int)
# extraBstrain0 = list({*singleMatchDF['bstrain_id_1'].values} - {*nonMatchDF['bstrain_id_0'].values})
# extraBstrain1 = list({*nonMatchDF['bstrain_id_0'].values} - {*singleMatchDF['bstrain_id_1'].values})
# extraVstrain0 = len(extraBstrain0) * [0]
# extraVstrain1 = len(extraBstrain1) * [0]
#
# nonMatchDF = pd.concat([nonMatchDF,pd.DataFrame({'bstrain_id_0':extraBstrain0, 'vstrain_id':extraVstrain0})],ignore_index=True)
# singleMatchDF = pd.concat([singleMatchDF,pd.DataFrame({'bstrain_id_1':extraBstrain1, 'vstrain_id':extraVstrain1})],ignore_index=True)
# nonMatchDF = nonMatchDF.astype(int)
# singleMatchDF = singleMatchDF.astype(int)
tripartiteDF = pd.merge(nonMatchDF, singleMatchDF, on="vstrain_id")
# totalVabund = curSim.execute("SELECT viral_abundance FROM summary \
# WHERE t = {0}".format(time)).fetchall()
# totalVabund = totalVabund[0][0]
# vFreqDict = {v:abund/totalVabund for (v,abund) in
#     curSim.execute("SELECT vstrain_id,abundance FROM vabundance \
#     WHERE t = {0} AND vstrain_id in ({1})".format(time,', '.join(map(str,vstrains0)))) }
vFreqDict = {v:abund for (v,abund) in
    curSim.execute("SELECT vstrain_id,abundance FROM vabundance \
    WHERE t = {0} AND vstrain_id in ({1})".format(time,', '.join(map(str,vstrains0)))) }
vFreqDict[0] = 0
vFreqList = [vFreqDict[vstrain] for vstrain in tripartiteDF['vstrain_id'].values]
tripartiteDF['vFrequency'] = vFreqList

MMnetwork = pd.pivot_table(tripartiteDF, values='vFrequency', index=['bstrain_id_0'], \
                    columns=['bstrain_id_1'], aggfunc=np.sum).fillna(0)
MMnetworkFull = MMnetwork.copy()

extraCols = set(MMnetwork.index.values) - set(MMnetwork.columns.values)
if len(extraCols) > 0:
    for i in list(extraCols):
        MMnetworkFull[i] = len(MMnetwork)*[0]

extraRows = set(MMnetwork.columns.values) - set(MMnetwork.index.values)
if len(extraRows) > 0:
    for i in list(extraRows):
        MMnetworkFull.loc[i] = len(MMnetwork.columns)*[0]
...
MMnetworkFull = MMnetworkFull.reindex(index = bstrainsAbundOrder[::-1])
MMnetworkFull = MMnetworkFull[bstrainsAbundOrder[::-1]]

bstrainsAbundOrder=[bstrain_id[0] for bstrain_id in
    curSim.execute("SELECT bstrain_id FROM babundance \
    WHERE t = {0} AND bstrain_id in ({1}) \
    ORDER BY abundance".format(time,', '.join(map(str,MMnetwork.columns.values))))]
MMnetwork = MMnetwork[bstrainsAbundOrder[::-1]]

bstrainsAbundOrder=[bstrain_id[0] for bstrain_id in
    curSim.execute("SELECT bstrain_id FROM babundance \
    WHERE t = {0} AND bstrain_id in ({1}) \
    ORDER BY abundance".format(time,', '.join(map(str,MMnetwork.index.values))))]
MMnetwork = MMnetwork.reindex(index = bstrainsAbundOrder[::-1])

edges = MMnetwork.stack().reset_index()
edges['bstrain_id_0'] = list(map(str,edges['bstrain_id_0']))
edges['bstrain_id_1'] = list(map(str,edges['bstrain_id_1']))
edges = edges.rename(columns={0: 'edge'})
edges = edges[edges.edge != 0]
edges['edge'] = np.log(edges['edge'])
edges = edges[edges.edge >= .1]
gcheck = ig.Graph.DataFrame(edges,directed=True)
visual_style = {}
visual_style["edge_width"] = [i/max(gcheck.es['edge']) for i in gcheck.es['edge']]
visual_style["layout"] = gcheck.layout("reingold_tilford", root=6)
ig.plot(gcheck, **visual_style)
ig.plot(gcheck, layout=layout)

        fig2, ax2 = plt.subplots(1,sharex=True)
        # cbar_kws=dict(pad=0.01,shrink=0.86, drawedges=True, ticks=range(int(maxfold)+1))
        ax2 = sns.heatmap(MMnetwork,
        xticklabels=True, yticklabels=True)

        ax2.set_xlabel(xlabel ='Microbe Strain ID',labelpad=10)
        ax2.set_ylabel(ylabel = 'Microbe Strain ID',labelpad=10)
        ax2.set_title('runID{0}-c{1}-r{2}\nMicrobe-Microbe Network at t = {3} (Time-ordered)'.format(run_id,combo_id,replicate,time),pad=20,fontsize=10)
        ...
        ax2.yaxis.set_tick_params(width=.25)
        # fig2.savefig(os.path.join(SCRIPT_PATH,'..','..','isolates',RUN_DIR,'projected-microbe-microbe-network-time-ordered-time{0}.{1}'.format(int(np.floor(time)),imgType)),dpi=resolve)
        for imgType in imgTypes:
            fig2.savefig(os.path.join('/Volumes/Yadgah','projected-microbe-microbe-network-abundance-ordered-time{0}.{1}'.format(int(np.floor(time)),imgType)),dpi=resolve)
...
#
#
# MMnetwork = MMnetwork.reindex(index = bstrains[::-1])
# MMnetwork = MMnetwork[bstrains]
#
#
#
# fig2, ax2 = plt.subplots(1,sharex=True)
# # cbar_kws=dict(pad=0.01,shrink=0.86, drawedges=True, ticks=range(int(maxfold)+1))
# ax2 = sns.heatmap(MMnetwork, xticklabels=True, yticklabels=True)
# ax2.set_xlabel(xlabel ='Microbe Strain ID',labelpad=10)
# ax2.set_ylabel(ylabel = 'Microbe Strain ID',labelpad=10)
# ax2.set_title('runID{0}-c{1}-r{2}\nMicrobe-Microbe Network at t = {3} (Phylogenetically Ordered)'.format(run_id,combo_id,replicate,time),pad=20,fontsize=10)
# # ax2 = sns.heatmap(matchNetwork,cmap=spacerCMAP, xticklabels=True, yticklabels=True)
# ax2.set_yticklabels(ax2.get_ymajorticklabels(), fontsize = 2)
# ax2.set_xticklabels(ax2.get_xmajorticklabels(), fontsize = 5)
# ax2.yaxis.set_tick_params(width=.25)
...
#
# # fig2.savefig(os.path.join(SCRIPT_PATH,'..','..','isolates',RUN_DIR,'projected-microbe-microbe-network-phylo-ordered-time{0}.{1}'.format(int(np.floor(time)),imgType)),dpi=resolve)
# for imgType in imgTypes:
#     fig2.savefig(os.path.join('/Volumes/Yadgah','projected-microbe-microbe-network-phylo-ordered-time{0}.{1}'.format(int(np.floor(time)),imgType)),dpi=resolve)
# plt.close(fig2)
#
#
#
