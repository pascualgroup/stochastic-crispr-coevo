...
# DBTREE_PATH = os.path.join(SCRIPT_PATH,'..','..','isolates',RUN_DIR,'probability-of-emergence_output.sqlite') # cluster
DBPROB_PATH = os.path.join('/Volumes','Yadgah',dir,'probability-of-emergence_output.sqlite') # local. run_id fixed; for testing
...
curProb.execute("SELECT DISTINCT t FROM match_phenotypes")
a_list = curProb.fetchall()
for i in a_list:
matchPhenotypes = pd.read_sql_query(
"SELECT t, time_specific_match_id, phenotypes \
FROM match_phenotypes", conProb)

for t in matchPhenotypes.t:
