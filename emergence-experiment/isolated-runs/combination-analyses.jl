...
if length(ARGS[2:end]) == 0
    error("please include combination ID")
end

cID = ARGS[2]
dbSim = SQLite.DB(dbSimPath)
run_ids = [run_id for (run_id,) 
    in execute(dbSim, "SELECT run_id FROM 
        runs WHERE combo_id = $(cID)")]
...
function main()
    # Root run directory
    if isfile(joinpath("isolates","jobs","combo-$(analysisType)jobs.sqlite"))
        rm(joinpath("isolates","jobs","combo-$(analysisType)jobs.sqlite"),force=true)
        println("deleted `/isolates/jobs/combo-$(analysisType)jobs.sqlite`.")
    end

    if ispath(joinpath("isolates","jobs","combo-$(analysisType)-jobs"))
        rm(joinpath("isolates","jobs","combo-$(analysisType)-jobs"),force=true,recursive=true)
        println("deleted `/isolates/jobs/combo-$(analysisType)-jobs`.")
    end
    ...
    if !ispath(joinpath("isolates","comboID$(cID)"))
        mkpath(joinpath("isolates","comboID$(cID)"))
    end
    dbSim = SQLite.DB(dbSimPath)
    # Create little database that corresponds analysis runs to jobIDs for troubleshooting
    dbTempJobs = SQLite.DB(joinpath("isolates","jobs","combo-$(analysisType)jobs.sqlite"))
...
function generate_analysis_runs(dbSim::DB) # This function generates the directories
    ...
    for run_id in run_ids
        (cr,) = execute(dbSim, "SELECT replicate FROM runs WHERE run_id = $(run_id)")
        run_dir = joinpath("isolates","comboID$(cID)", "runs","runID$(run_id)-r$(cr[1])")
        if isfile(joinpath(run_dir,"$(analysisType)_output.sqlite"))
            error("Please move, delete, or rename `isolates/comboID$(cID)/runs/runID$(run_id)-r$(cr[1])/$(analysisType)_output.sqlite`.")
        end
        if !ispath(run_dir)
            mkpath(run_dir)
        end
        ...
        if length(ARGS[3:end]) > 0
            ...
            open(run_script, "w") do f
                ...
                #!/bin/sh
                cd `dirname \$0`
                julia $(ROOT_RUN_SCRIPT) $(run_id) combo &> run-output-$(analysisType).txt
                """)
            end
        end
...
function generate_analysis_jobs(dbSim::DB,dbTempJobs::DB,numSubmits::Int64)
    ...
    for script in 1:numSubmits
        push!(submitScripts,open("$(script)_combo-$(analysisType)-submit-jobs.sh", "w"))
    ...
    for (job_id,) in execute(dbTempJobs, "SELECT DISTINCT job_id FROM job_runs ORDER BY job_id")
        ...
        job_dir = joinpath("isolates","jobs","combo-$(analysisType)-jobs", "$(job_id)")
    ...
    @info "
    Sweep will be submitted via $(numSubmits) `combo-$(analysisType)-submit-jobs.sh` script(s).
    Each `combo-analysis_submit_jobs.sh` script submits $(N_JOBS_MAX) job(s).
